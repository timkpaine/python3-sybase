...
{
    if (PyLong_Check(obj))
	return money_from_int(money, type, PyLong_AsLong(obj));
    else if (PyLong_Check(obj))
	return money_from_long(money, type, obj);
    else if (PyFloat_Check(obj))
	return money_from_float(money, type, PyFloat_AsDouble(obj));
    else if (PyUnicode_Check(obj))
	return money_from_string(money, type, PyUnicode_AsUTF8(obj));
...
MoneyObj *Money_FromInt(PyObject *obj, int type)
{
    ...
    if (money_from_int(&money, type, PyLong_AsLong(obj)))
...
static PyMemberDef Money_memberlist[] = {
...
PyTypeObject MoneyType = {
    ...
    0,				/*tp_clear*/
#ifdef HAVE_CS_CMP
    (richcmpfunc)Money_compare,	/*tp_compare*/
#else
    0,				/*tp_compare*/
#endif
    0,				/*tp_weaklistoffset*/
