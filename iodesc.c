...
static PyMemberDef CS_IODESC_memberlist[] = {
...
static PyObject *CS_IODESC_getattr(CS_IODESCObj *self, char *name)
{
	...
	return PyUnicode_FromStringAndSize(self->iodesc.name,
					  self->iodesc.namelen);
    if (strcmp(name, "timestamp") == 0)
        return PyUnicode_FromStringAndSize((char *)self->iodesc.timestamp,
					  self->iodesc.timestamplen);
    if (strcmp(name, "textptr") == 0)
	return PyUnicode_FromStringAndSize((char *)self->iodesc.textptr,
...
static int CS_IODESC_setattr(CS_IODESCObj *self, char *name, PyObject *v)
{
	...
	memmove(ptr, PyUnicode_AsUTF8(v), size);
	*len_ptr = size;
	return 0;
    }
