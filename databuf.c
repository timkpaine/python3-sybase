...
PyObject *databuf_alloc(PyObject *obj)
{
	...
	if (PyLong_Check(obj) || obj == Py_None)
	    /* if (PyLong_AsLong(obj) <= INT32_MAX && PyLong_AsLong(obj) >= INT32_MIN) */
	    ...
	    self->fmt.maxlength = PyUnicode_GetSize(obj) + 1;
#ifdef HAVE_DATETIME
...
static PyObject *DataBuf_item(PyObject *_self, Py_ssize_t i)
{
    ...
    case CS_BINARY_TYPE:
	return PyUnicode_FromStringAndSize(item, self->copied[i]);
	...
	if (self->strip) {
		...
		if (((char*)item)[end] != ' ')
		    break;
	    return PyUnicode_FromStringAndSize(item, end + 1);
	} else
	    return PyUnicode_FromStringAndSize(item, self->copied[i]);

    case CS_BIT_TYPE:
	return PyLong_FromLong(*(CS_BIT*)item);

    case CS_TINYINT_TYPE:
	return PyLong_FromLong(*(CS_TINYINT*)item);

    case CS_SMALLINT_TYPE:
	return PyLong_FromLong(*(CS_SMALLINT*)item);

    case CS_INT_TYPE:
	return PyLong_FromLong(*(CS_INT*)item);
...
static Py_ssize_t DataBuf_ass_item(PyObject *_self, Py_ssize_t i, PyObject *v)
{
	...
	*(CS_BIT*)item = (CS_BIT)PyLong_AsLong(v);
	...
	*(CS_TINYINT*)item = (CS_TINYINT)PyLong_AsLong(v);
	self->copied[i] = self->fmt.maxlength;
	break;
	...
	*(CS_SMALLINT*)item = (CS_SMALLINT)PyLong_AsLong(v);
	self->copied[i] = self->fmt.maxlength;
	break;

    case CS_INT_TYPE:
        if (PyUnicode_Check(v)) {
	    obj = PyLong_FromString(PyUnicode_AsUTF8(v), NULL, 0);
	    if (obj == NULL) {
	    	return -1;
	    }
	    v = obj;
...
static PyMemberDef DataBuf_memberlist[] = {
    { "name", T_STRING_INPLACE, OFF(fmt.name) }, /* faked */
    { "datatype", T_INT, OFF(fmt.datatype), READONLY },
    { "format", T_INT, OFF(fmt.format) },
    { "maxlength", T_INT, OFF(fmt.maxlength), READONLY },
    { "scale", T_INT, OFF(fmt.scale), READONLY },
    { "precision", T_INT, OFF(fmt.precision), READONLY },
    { "status", T_INT, OFF(fmt.status) },
    { "count", T_INT, OFF(fmt.count), READONLY },
    { "usertype", T_INT, OFF(fmt.usertype), READONLY },
...
static PyObject *DataBuf_getattr(DataBufObj *self, char *name)
{
	...
	return PyUnicode_FromStringAndSize(self->fmt.name, self->fmt.namelen);

    rv = PyObject_GenericGetAttr((PyObject *)self, name);
    if (rv)
	return rv;
    PyErr_Clear();
...
static int DataBuf_setattr(DataBufObj *self, char *name, PyObject *v)
{
	...
	strncpy(self->fmt.name, PyUnicode_AsUTF8(v), sizeof(self->fmt.name));
