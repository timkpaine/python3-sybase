...
sybase = None
if "SYBASE" in os.environ:
    sybase = os.environ['SYBASE']
    if "SYBASE_OCS" in os.environ:
...
for i in range(1, len(sys.argv)):
    ...
    parts = sys.argv[i].split('HAVE_FREETDS')
    if len(parts) == 1:
        continue
...
for api in ('blk_alloc', 'blk_describe', 'blk_drop', 'blk_rowxfer_mult',
    ...
    if api_exists(api, os.path.join(syb_incdir, 'bkpublic.h')):
        syb_macros.append(('HAVE_' + api.upper(), None))
...
class PreReleaseCheck:
    def __init__(self, distribution):
        ...
        self.check_rev('sybasect.c', r'rev = PyUnicode_FromString\("(.*)"\)')
