...
int numeric_from_value(CS_NUMERIC *num, int precision, int scale, PyObject *obj)
{
    ...
    if (PyLong_Check(obj))
	return numeric_from_int(num, precision, scale, PyLong_AsLong(obj));
	...
	return numeric_from_string(num, precision, scale, PyUnicode_AsUTF8(obj));
    else if (Numeric_Check(obj))
	return numeric_from_numeric(num, precision, scale,
				    &((NumericObj*)obj)->num);
    else if (pydecimal_check(obj)) {
	str = PyObject_Str(obj);
	res = numeric_from_string(num, precision, scale, PyUnicode_AsUTF8(str));
	Py_DECREF(str);
	return res;
    }						   
...
NumericObj *Numeric_FromInt(PyObject *obj, int precision, int scale)
{
    ...
    if (numeric_from_int(&num, precision, scale, PyLong_AsLong(obj)))
...
NumericObj *Numeric_FromString(PyObject *obj, int precision, int scale)
{
    ...
    if (numeric_from_string(&num, precision, scale, PyUnicode_AsUTF8(obj)))
	return numeric_alloc(&num);
    return NULL;
}
...
static PyMemberDef Numeric_memberlist[] = {
    { "precision", T_BYTE, OFF(num.precision), READONLY },
    { "scale",     T_BYTE, OFF(num.scale), READONLY },
    { NULL }			/* Sentinel */
};
...
PyTypeObject NumericType = {
    ...
    0,				/*tp_clear*/
#ifdef HAVE_CS_CMP
    (richcmpfunc)Numeric_compare,	/*tp_compare*/
#else
    0,				/*tp_compare*/
#endif
    0,				/*tp_weaklistoffset*/
