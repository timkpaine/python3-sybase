...
static PyObject *sybasect_CS_LAYER(PyObject *module, PyObject *args)
{
    ...
    return PyLong_FromLong(CS_LAYER(num));
}
...
static PyObject *sybasect_CS_ORIGIN(PyObject *module, PyObject *args)
{
    ...
    return PyLong_FromLong(CS_ORIGIN(num));
}
...
static PyObject *sybasect_CS_SEVERITY(PyObject *module, PyObject *args)
{
    ...
    return PyLong_FromLong(CS_SEVERITY(num));
}
...
static PyObject *sybasect_CS_NUMBER(PyObject *module, PyObject *args)
{
    ...
    return PyLong_FromLong(CS_NUMBER(num));
}
...
static struct PyModuleDef moduledef = {
    ...
    .m_name = "sybasect",
    .m_doc = PyDoc_STR(sybasect_module_documentation),
    .m_size = -1,
    .m_methods = sybasect_methods
};

PyMODINIT_FUNC PyInit_sybasect(void)
{
...
#ifdef WANT_BULKCOPY
    Py_TYPE(&CS_BLKDESCType) = &PyType_Type;
#endif
    Py_TYPE(&CS_COMMANDType) = &PyType_Type;
    Py_TYPE(&CS_CONNECTIONType) = &PyType_Type;
    Py_TYPE(&CS_CONTEXTType) = &PyType_Type;
    Py_TYPE(&CS_DATAFMTType) = &PyType_Type;
    Py_TYPE(&CS_IODESCType) = &PyType_Type;
    Py_TYPE(&CS_LOCALEType) = &PyType_Type;
    Py_TYPE(&CS_CLIENTMSGType) = &PyType_Type;
    Py_TYPE(&CS_SERVERMSGType) = &PyType_Type;
    Py_TYPE(&NumericType) = &PyType_Type;
    Py_TYPE(&DateTimeType) = &PyType_Type;
#ifdef CS_DATE_TYPE
    Py_TYPE(&DateType) = &PyType_Type;
#endif
    Py_TYPE(&MoneyType) = &PyType_Type;
    Py_TYPE(&DataBufType) = &PyType_Type;
...
#ifdef HAVE_DECIMAL
    builtins = PyImport_AddModule("builtins"); if (!builtins) goto error;
    __import__ = PyObject_GetAttrString(builtins, "__import__"); if (!__import__) goto error;
    Decimal = PyUnicode_FromString("Decimal"); if (!Decimal) goto error;
    ...
    if ((rev = PyUnicode_FromString("0.40")) == NULL)
	goto error;
