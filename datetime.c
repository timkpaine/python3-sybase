...
static PyNumberMethods DateTime_as_number = {
    ...
    (binaryfunc)0,		/*nb_xor*/
    (binaryfunc)0,		/*nb_or*/
    (unaryfunc)DateTime_long,	/*nb_int*/
    0,                      /*nb_reserved*/
...
static PyMemberDef DateTime_memberlist[] = {
    { "type",      T_INT, OFFSET(type), READONLY },
    { "year",      T_INT, OFFSET(daterec.dateyear), READONLY },
    { "month",     T_INT, OFFSET(daterec.datemonth), READONLY },
    { "day",       T_INT, OFFSET(daterec.datedmonth), READONLY },
    { "dayofyear", T_INT, OFFSET(daterec.datedyear), READONLY },
    { "weekday",   T_INT, OFFSET(daterec.datedweek), READONLY },
    { "hour",      T_INT, OFFSET(daterec.datehour), READONLY },
    { "minute",    T_INT, OFFSET(daterec.dateminute), READONLY },
    { "second",    T_INT, OFFSET(daterec.datesecond), READONLY },
    { "msecond",   T_INT, OFFSET(daterec.datemsecond), READONLY },
    { "tzone",     T_INT, OFFSET(daterec.datetzone), READONLY },
    { NULL }
};
...
static PyObject *DateTime_getattr(DateTimeObj *self, char *name)
{
    ...
    rv = PyObject_GenericGetAttr((PyObject*)self, PyUnicode_FromString(name));
    if (rv)
	return rv;
    PyErr_Clear();
...
static int DateTime_setattr(DateTimeObj *self, char *name, PyObject *v)
{
    ...
    return PyObject_GenericSetAttr((PyObject*)self, PyUnicode_FromString(name), v);
}
